...
for i in range(COUNT_EXPERIMENTS_GLOBAL):
    ...
    for param_dict in param_dict_list:
        ...
        if TRIGGER_OF_CALCULATION_INTEGRAL_INDICATORS_ON_EACH_BOT is True:
            ...
            df['return_bot_procent'] = df['day_profit_unrealized_pnl'] / df['reserved_sum_investment']
            ...
            convert_day_stock_df_to_week(df)
            convert_day_stock_df_to_month(df)

        # запись в файл бота
        path_bot_i = path_file(path_bot, i_cumulative_param + 1)  # с каждым новым рандомным параметром увеличивается индекс файла
