...
for j in range(1, len(date_list)):
    ...
    for i in range(COUNT_EXPERIMENTS_GLOBAL):
        ...
        if k in range(len(df_ticker_i)):
            ...
            if date_j in df_ticker_i['Time'].tolist() and first_step_status_list[i] == True:
            ...
            elif date_j in df_ticker_i['Time'].tolist() and first_step_status_list[i] == False:
                # print('date_j = ', date_j, first_step_status)
                # print(k_list[i] + 1)
                # k = k_list[i]
                ...
                start_signal = True
                df_ticker_i = bot_generator_restart_day(df_ticker_i, param_dict, k, start_signal)
